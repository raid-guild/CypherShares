...
import { Controller, CSToken, StandardTokenMock, ModuleBaseMock } from "@utils/contracts";
...
describe("CSToken", () => {
    ...
    describe("constructor", async () => {
        ...
        beforeEach(async () => {
            ...
            subjectName = "TestCSToken";
            subjectSymbol = "SET";
        });

        async function subject(): Promise<any> {
            return await deployer.core.deployCSToken(
        ...
        it("should have the correct name, symbol, controller, multiplier, and manager", async () => {
            const csToken = await subject();

            const name = await csToken.name();
            const symbol = await csToken.symbol();
            const controllerAddress = await csToken.controller();
            const managerAddress = await csToken.manager();
            const positionMultiplier = await csToken.positionMultiplier();
        ...
        it("should have the correct components and componentPositions", async () => {
            ...
            const firstComponent = await csToken.components(0);
            const secondComponent = await csToken.components(1);
            const firstComponentVirtualUnit = await csToken.getDefaultPositionRealUnit(firstComponent);
            const secondComponentVirtualUnit = await csToken.getDefaultPositionRealUnit(secondComponent);
            const firstComponentExternalModules = await csToken.getExternalPositionModules(firstComponent);
            const secondComponentExternalModules = await csToken.getExternalPositionModules(secondComponent);
        ...
        it("should have the 0 modules initialized", async () => {
            ...
            const modules = await csToken.getModules();
        ...
        it("should have the correct modules in pending state", async () => {
            ...
            const mockBasicIssuanceModuleState = await csToken.moduleStates(mockBasicIssuanceModule.address);
            const mockLockedModuleState = await csToken.moduleStates(mockLockedModule.address);
            expect(mockBasicIssuanceModuleState).to.eq(MODULE_STATE["PENDING"]);
            expect(mockLockedModuleState).to.eq(MODULE_STATE["PENDING"]);
        });
    });

    context("when there is a deployed CSToken", async () => {
        let csToken: CSToken;
        ...
        beforeEach(async () => {
            ...
            csToken = await deployer.core.deployCSToken(
                components,
                units,
                modules,
                controller.address,
            ...
            csToken = csToken.connect(mockBasicIssuanceModule.wallet);
            await csToken.initializeModule();

            csToken = csToken.connect(mockLockedModule.wallet);
            await csToken.initializeModule();
        });
        ...
        describe("#invoke", async () => {
            ...
            async function subject(): Promise<any> {
                csToken = csToken.connect(subjectCaller.wallet);
                return csToken.invoke(
            ...
            it("should set the CSTokens approval balance to the spender", async () => {
                await subject();

                const allowance = await firstComponent.allowance(csToken.address, testSpender);
            ...
            it("should emit the Invoked event", async () => {
                ...
                await expect(subject()).to.emit(csToken, "Invoked").withArgs(
                    subjectTargetAddress,
                    subjectValue,
                    subjectCallData,
                    expectedReturnValue
            ...
            describe("when the module is locked", async () => {
                beforeEach(async () => {
                    await csToken.connect(mockLockedModule.wallet).lock();
            ...
            describe("when sending ETH to a CSToken contract", async () => {
                let transferBalance: BigNumber;
                let csTokenReceivingETH: CSToken;

                beforeEach(async () => {
                    csTokenReceivingETH = await deployer.core.deployCSToken(
                        components,
                        units,
                        modules,
                        controller.address,
                    ...
                    await manager.wallet.sendTransaction({ to: csToken.address, value: transferBalance });

                    subjectCallData = EMPTY_BYTES;
                    subjectTargetAddress = csTokenReceivingETH.address;
        ...
        describe("#addComponent", async () => {
            ...
            async function subject(): Promise<any> {
                return csToken.connect(subjectCaller.wallet).addComponent(subjectComponent);
            }

            it("should add to the component array", async () => {
                const prevComponents = await csToken.getComponents();

                await subject();

                const components = await csToken.getComponents();

                const expectedComponent = await csToken.components(components.length - 1);
            ...
            it("should emit the ComponentAdded event", async () => {
                await expect(subject()).to.emit(csToken, "ComponentAdded").withArgs(subjectComponent);
            });

            shouldRevertIfModuleDisabled(subject);
            shouldRevertIfCallerIsNotModule(subject);
            shouldRevertIfCSTokenIsLocked(subject);
        });
        ...
        describe("#removeComponent", async () => {
            ...
            async function subject(): Promise<any> {
                return csToken.connect(subjectCaller.wallet).removeComponent(subjectComponent);
            }
            ...
            it("should emit the ComponentRemoved event", async () => {
                await expect(subject()).to.emit(csToken, "ComponentRemoved").withArgs(subjectComponent);
            });
        ...
        describe("#editDefaultPositionUnit", async () => {
            ...
            beforeEach(async () => {
                ...
                await csToken.connect(subjectCaller.wallet).editPositionMultiplier(multiplier);
            });

            async function subject(): Promise<any> {
                return csToken.connect(subjectCaller.wallet).editDefaultPositionUnit(subjectComponent, subjectNewUnit);
            }

            it("should properly edit the default position unit", async () => {
                await subject();

                const retrievedUnit = await csToken.getDefaultPositionRealUnit(subjectComponent);
                expect(retrievedUnit).to.eq(subjectNewUnit);
            });

            it("should emit the DefaultPositionUnitEdited event", async () => {
                await expect(subject()).to.emit(csToken, "DefaultPositionUnitEdited").withArgs(
                    subjectComponent,
                    subjectNewUnit
                );
            });
            ...
            describe("when the conversion results in a virtual unit of 0", async () => {
                beforeEach(async () => {
                    ...
                    await csToken.connect(subjectCaller.wallet).editPositionMultiplier(hugePositionMultiplier);
                });
        ...
        describe("#addExternalPositionModule", async () => {
            ...
            async function subject(): Promise<any> {
                return csToken.connect(subjectCaller.wallet).addExternalPositionModule(subjectComponent, subjectExternalModule);
            }

            it("should properly add the module", async () => {
                const prevModules = await csToken.getExternalPositionModules(subjectComponent);

                await subject();

                const retrievedExternalModules = await csToken.getExternalPositionModules(subjectComponent);
            ...
            it("should emit the PositionModuleAdded event", async () => {
                await expect(subject()).to.emit(csToken, "PositionModuleAdded").withArgs(subjectComponent, subjectExternalModule);
            });
        ...
        describe("#removeExternalPositionModule", async () => {
            ...
            beforeEach(async () => {
                ...
                await csToken.connect(subjectCaller.wallet).addExternalPositionModule(subjectComponent, subjectExternalModule);
            });

            async function subject(): Promise<any> {
                return csToken.connect(subjectCaller.wallet).removeExternalPositionModule(subjectComponent, subjectExternalModule);
            }
            ...
            it("should zero out the data in externalPositions", async () => {
                ...
                const retrievedRealUnit = await csToken.getExternalPositionRealUnit(subjectComponent, subjectExternalModule);
                const retrievedData = await csToken.getExternalPositionData(subjectComponent, subjectExternalModule);
            ...
            it("should emit the PositionModuleRemoved event", async () => {
                await expect(subject()).to.emit(csToken, "PositionModuleRemoved").withArgs(subjectComponent, subjectExternalModule);
            });
        ...
        describe("#editExternalPositionUnit", async () => {
            ...
            async function subject(): Promise<any> {
                return csToken.connect(subjectCaller.wallet).editExternalPositionUnit(
                    subjectComponent,
                    subjectModule,
                    subjectNewUnit
                );
            ...
            it("should properly edit the external position unit", async () => {
                ...
                const retrievedUnit = await csToken.getExternalPositionRealUnit(subjectComponent, subjectModule);
                expect(retrievedUnit).to.eq(subjectNewUnit);
            });

            it("should emit the ExternalPositionUnitEdited event", async () => {
                await expect(subject()).to.emit(csToken, "ExternalPositionUnitEdited").withArgs(
                    subjectComponent,
                    subjectModule,
                    subjectNewUnit
                );
        ...
        describe("#editExternalPositionData", async () => {
            ...
            async function subject(): Promise<any> {
                return csToken.connect(subjectCaller.wallet).editExternalPositionData(
                    subjectComponent,
                    subjectModule,
                    subjectData
                );
            ...
            it("should properly edit the external position unit", async () => {
                ...
                const data = await csToken.getExternalPositionData(subjectComponent, subjectModule);
                expect(data).to.eq(subjectData);
            });

            it("should emit the ExternalPositionDataEdited event", async () => {
                await expect(subject()).to.emit(csToken, "ExternalPositionDataEdited").withArgs(
                    subjectComponent,
                    subjectModule,
                    subjectData
                );
        ...
        describe("#editPositionMultiplier", async () => {
            ...
            async function subject(): Promise<any> {
                return csToken.connect(subjectCaller.wallet).editPositionMultiplier(subjectPositionMultiplier);
            }
            ...
            it("should update the real position units", async () => {
                ...
                const firstPositionRealUnits = await csToken.getDefaultPositionRealUnit(firstComponent.address);
            ...
            it("should emit the correct PositionMultiplierEdited event", async () => {
                await expect(subject()).to.emit(csToken, "PositionMultiplierEdited").withArgs(subjectPositionMultiplier);
            });
        ...
        describe("#lock", async () => {
            ...
            it("should lock the CSToken", async () => {
                await subject();

                const isLocked = await csToken.isLocked();
            ...
            it("should set the locker to the module", async () => {
                ...
                const lockerAddress = await csToken.locker();
            ...
            describe("when the CSToken is already locked", async () => {
                beforeEach(async () => {
                    csToken = csToken.connect(subjectCaller.wallet);
                    await csToken.lock();
                });
        ...
        describe("#unlock", async () => {
            ...
            async function subject(): Promise<any> {
                csToken = csToken.connect(subjectCaller.wallet);
                return csToken.unlock();
            }

            it("should put the CSToken in an unlocked state", async () => {
                await subject();
            ...
            describe("when the CSToken is already unlocked", async () => {
                beforeEach(async () => {
                    await csToken.unlock();
                });
        ...
        describe("#mint", async () => {
            ...
            async function subject(): Promise<any> {
                csToken = csToken.connect(subjectCaller.wallet);
                return csToken.mint(subjectMintee, subjectQuantity);
            }

            it("should mint the correct quantity to the mintee", async () => {
                await subject();

                const newSetBalance = await csToken.balanceOf(subjectMintee);
        ...
        describe("#burn", async () => {
            ...
            beforeEach(async () => {
                ...
                csToken = csToken.connect(subjectCaller.wallet);
                await csToken.mint(subjectMintee, mintQuantity);
            });

            async function subject(): Promise<any> {
                csToken = csToken.connect(subjectCaller.wallet);
                return csToken.burn(subjectMintee, subjectQuantity);
            }
        ...
        describe("#addModule", async () => {
            ...
            async function subject(): Promise<any> {
                csToken = csToken.connect(subjectCaller.wallet);
                return csToken.addModule(subjectModule);
            }

            it("should change the state to pending", async () => {
                await subject();

                const moduleState = await csToken.moduleStates(subjectModule);
                expect(moduleState).to.eq(MODULE_STATE["PENDING"]);
            });

            it("should emit the ModuleAdded event", async () => {
                await expect(subject()).to.emit(csToken, "ModuleAdded").withArgs(subjectModule);
            });
        ...
        describe("#removeModule", async () => {
            ...
            beforeEach(async () => {
                ...
                csToken = csToken.connect(manager.wallet);
                await csToken.addModule(moduleMock.address);
                await moduleMock.initializeModuleOnSet(csToken.address);
            ...
            it("should emit the ModuleRemoved event", async () => {
                await expect(subject()).to.emit(csToken, "ModuleRemoved").withArgs(subjectModule);
            });
            ...
            describe("when the module is pending", async () => {
                beforeEach(async () => {
                    await controller.addModule(pendingModule.address);
                    await csToken.connect(manager.wallet).addModule(pendingModule.address);
        ...
        describe("#removePendingModule", async () => {
            ...
            async function subject(): Promise<any> {
                csToken = csToken.connect(subjectCaller.wallet);
                return csToken.removePendingModule(subjectModule);
            }
            ...
            it("should emit the PendingModuleRemoved event", async () => {
                await expect(subject()).to.emit(csToken, "PendingModuleRemoved").withArgs(subjectModule);
            });
        ...
        describe("#setManager", async () => {
            ...
            async function subject(): Promise<any> {
                csToken = csToken.connect(subjectCaller.wallet);
                return csToken.setManager(subjectManager);
            }
            ...
            it("should emit the ManagerEdited event", async () => {
                await expect(subject()).to.emit(csToken, "ManagerEdited").withArgs(subjectManager, manager.address);
            });
        ...
        describe("#initializeModule", async () => {
            ...
            beforeEach(async () => {
                ...
                await csToken.addModule(subjectModule);
            });

            async function subject(): Promise<any> {
                csToken = csToken.connect(subjectCaller.wallet);
                return csToken.initializeModule();
            }

            it("should add the module to the modules list", async () => {
                await subject();

                const moduleList = await csToken.getModules();
            ...
            it("should emit the ModuleInitialized event", async () => {
                await expect(subject()).to.emit(csToken, "ModuleInitialized").withArgs(subjectModule);
            });
        ...
        describe("#getDefaultPositionRealUnit", async () => {
            ...
            beforeEach(async () => {
                ...
                await csToken.connect(mockBasicIssuanceModule.wallet).editPositionMultiplier(multiplier);
            });

            async function subject(): Promise<BigNumber> {
                return await csToken.getDefaultPositionRealUnit(subjectComponent);
            }
        ...
        describe("#getExternalPositionRealUnit", async () => {
            ...
            beforeEach(async () => {
                ...
                await csToken.connect(mockBasicIssuanceModule.wallet).editExternalPositionUnit(
                    subjectComponent,
                    subjectModule,
                    externalUnitToAdd
                );
            ...
            async function subject(): Promise<BigNumber> {
                return await csToken.getExternalPositionRealUnit(subjectComponent, subjectModule);
            }
        ...
        describe("#getComponents", async () => {
            async function subject(): Promise<Address[]> {
                return await csToken.getComponents();
            }
        ...
        describe("#getExternalPositionModules", async () => {
            ...
            beforeEach(async () => {
                ...
                await csToken.connect(mockBasicIssuanceModule.wallet).addExternalPositionModule(
                    subjectComponent,
            ...
            async function subject(): Promise<Address[]> {
                return await csToken.getExternalPositionModules(subjectComponent);
            }
        ...
        describe("#getExternalPositionData", async () => {
            ...
            beforeEach(async () => {
                ...
                await csToken.connect(subjectCaller.wallet).editExternalPositionData(
                    subjectComponent,
                    subjectModule,
                    expectedData
                );
            ...
            async function subject(): Promise<string> {
                return csToken.getExternalPositionData(
                    subjectComponent,
                    subjectModule,
                );
            }
        ...
        describe("#getPositions", async () => {
            let subjectCSToken: CSToken;

            const subjectMultiplier = ether(0.5);

            beforeEach(async () => {
                subjectCSToken = csToken;

                await subjectCSToken.connect(mockBasicIssuanceModule.wallet).editPositionMultiplier(subjectMultiplier);
            });

            async function subject(): Promise<Position[]> {
                return await subjectCSToken.getPositions();
            }
            ...
            describe("when the CSToken has an external position and the Default virtual unit is 0", async () => {
                ...
                beforeEach(async () => {
                    ...
                    await subjectCSToken.connect(mockBasicIssuanceModule.wallet).addComponent(externalComponent);
                    // Add module to the component
                    await subjectCSToken.connect(mockBasicIssuanceModule.wallet).addExternalPositionModule(
                        externalComponent,
                        externalModule
                    );
                    await subjectCSToken.connect(mockBasicIssuanceModule.wallet).editExternalPositionUnit(
                        externalComponent,
                        externalModule,
                        externalRealUnit,
                    );

                    await subjectCSToken.connect(mockBasicIssuanceModule.wallet).editExternalPositionData(
                        externalComponent,
                        externalModule,
                        externalData,
                    );
        ...
        describe("#getModules", async () => {
            async function subject(): Promise<Address[]> {
                return await csToken.getModules();
            }
        ...
        describe("#getTotalComponentRealUnits", async () => {
            ...
            async function subject(): Promise<BigNumber> {
                return await csToken.getTotalComponentRealUnits(subjectComponent);
            }
        ...
        describe("#isInitializedModule", async () => {
            ...
            async function subject(): Promise<boolean> {
                return await csToken.isInitializedModule(subjectModule);
            }
        ...
        function shouldRevertIfCSTokenIsLocked(subject: any) {
            describe("when the CSToken is locked", async () => {
                beforeEach(async () => {
                    await csToken.connect(mockLockedModule.wallet).lock();
                });
