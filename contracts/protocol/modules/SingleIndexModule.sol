...
contract SingleIndexModule is ModuleBase, ReentrancyGuard {
    ...
    ICSToken public index;                                 // Index being managed with contract
     ...
     */
    function initialize(ICSToken _index)
        external
        ...
        onlyValidAndPendingSet(_index)
    {
        ...
        ICSToken.Position[] memory positions = _index.getPositions();

        for (uint256 i = 0; i < positions.length; i++) {
            ICSToken.Position memory position = positions[i];
