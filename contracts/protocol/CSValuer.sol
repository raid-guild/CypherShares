 ...
 * @author Set Protocol
 *
 * Contract that returns the valuation of CSTokens using price oracle data used in contracts
...
contract CSValuer {
    ...
    /**
     * Gets the valuation of a CSToken using data from the price oracle. Reverts
     * if no price exists for a component in the CSToken. Note: this works for external
     ...
     *
     * @param _csToken        CSToken instance to get valuation
     * @param _quoteAsset      Address of token to quote valuation in
     *
     * @return                 CSToken valuation in terms of quote asset in precise units 1e18
     */
    function calculateCSTokenValuation(ICSToken _csToken, address _quoteAsset) external view returns (uint256) {
        ...
        for (uint256 i = 0; i < components.length; i++) {
            ...
            int256 aggregateUnits = _csToken.getTotalComponentRealUnits(component);
