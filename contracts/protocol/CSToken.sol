...
contract CSToken is ERC20 {
    ...
    /*
        The PositionState is the status of the Position, whether it is Default (held on the CSToken)
    ...
    */
    ...
    /**
     * Throws if the sender is not a CSToken's module or module not enabled
     */
    modifier onlyModule() {
    ...
    /**
     * Throws if the sender is not the CSToken's manager
     */
    ...
    /**
     * Throws if CSToken is locked and called by any account other than the locker.
     */
    ...
    // List of initialized Modules; Modules extend the functionality of CSTokens
    ...
    mapping(address => ICSToken.ModuleState) public moduleStates;
    ...
    mapping(address => ICSToken.ComponentPosition) private componentPositions;
    ...
    /**
     * When a new CSToken is created, initializes Positions in default state and adds modules into pending state.
     * All parameter validations are on the CSTokenCreator contract. Validations are performed already on the 
     * CSTokenCreator. Initiates the positionMultiplier as 1e18 (no adjustments).
     *
        ...
        ERC20(_name, _symbol)
    {
        ...
        for (uint256 i = 0; i < _modules.length; i++) {
            moduleStates[_modules[i]] = ICSToken.ModuleState.PENDING;
        }
    ...
    /**
     * PRIVELEGED MODULE FUNCTION. When a CSToken is locked, only the locker can call privileged functions.
     */
    ...
    /**
     * PRIVELEGED MODULE FUNCTION. Unlocks the CSToken and clears the locker
     */
    ...
    function addModule(address _module) external onlyManager {
        require(moduleStates[_module] == ICSToken.ModuleState.NONE, "Module must not be added");
        require(controller.isModule(_module), "Must be enabled on Controller");

        moduleStates[_module] = ICSToken.ModuleState.PENDING;

        emit ModuleAdded(_module);
    }

    /**
     * MANAGER ONLY. Removes a module from the CSToken. CSToken calls removeModule on module itself to confirm
    ...
    function removeModule(address _module) external onlyManager {
        ...
        require(moduleStates[_module] == ICSToken.ModuleState.INITIALIZED, "Module must be added");

        IModule(_module).removeModule();

        moduleStates[_module] = ICSToken.ModuleState.NONE;
    ...
    /**
     * MANAGER ONLY. Removes a pending module from the CSToken.
     */
    function removePendingModule(address _module) external onlyManager {
        require(!isLocked, "Only when unlocked");
        require(moduleStates[_module] == ICSToken.ModuleState.PENDING, "Module must be pending");
    ...
    function initializeModule() external {
        require(!isLocked, "Only when unlocked");
        require(moduleStates[msg.sender] == ICSToken.ModuleState.PENDING, "Module must be pending");
        
        moduleStates[msg.sender] = ICSToken.ModuleState.INITIALIZED;
    ...
    /**
     * MANAGER ONLY. Changes manager; We allow null addresses in case the manager wishes to wind down the CSToken.
    ...
    function isInitializedModule(address _module) external view returns (bool) {
        return moduleStates[_module] == ICSToken.ModuleState.INITIALIZED;
    }
    ...
    function isPendingModule(address _module) external view returns (bool) {
        return moduleStates[_module] == ICSToken.ModuleState.PENDING;
    }
     ...
     */
    function getPositions() external view returns (ICSToken.Position[] memory) {
        ICSToken.Position[] memory positions = new ICSToken.Position[](_getPositionCount());
        ...
        for (uint256 i = 0; i < components.length; i++) {
            ...
            if (_defaultPositionVirtualUnit(component) > 0) {
                positions[positionCount] = ICSToken.Position({
                    component: component,
                    module: address(0),
     ...
     *
     * Module must be initialized on the CSToken and enabled by the controller
     */
